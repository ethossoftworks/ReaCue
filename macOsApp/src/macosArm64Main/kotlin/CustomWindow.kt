...
/**
 * Replaces the Compose-provided Window composable with a version that fixes two bugs present in Compose Multiplatform
 * 1.11.0:
 *
 * 1. Title bar flash on launch: ComposeWindow calls makeKeyAndOrderFront before setContent, so the window is visible
 *    before any composition runs. We configure the chrome flags (FullSizeContentView, titleVisibility,
 *    titlebarAppearsTransparent) before the window is ordered front so they take effect on the very first frame.
 *
 * 2. Hit-box drift after monitor change: ComposeWindow sets scene.density once from backingScaleFactor at init time and
 *    never updates it. Mouse event coordinates are converted via event.offset.toOffset(scene.density), so a stale
 *    density causes every click to land at the wrong logical pixel after moving to a display with different DPI. We
 *    observe NSWindowDidChangeBackingPropertiesNotification and update scene.density, which also cascades to
 *    RootNodeOwner (and therefore LocalDensity) through the CanvasLayersComposeScene setter.
 *
 * Trade-off vs the stock implementation: MacosTextInputService (internal) is omitted, so IME composition (e.g.
 * Chinese/Japanese input) is not supported. Basic typing still works via keyDown/keyUp key events.
 */
fun CustomWindow(
...
private class AppWindow(title: String, size: DpSize, minSize: DpSize, content: @Composable WindowScope.() -> Unit) :
    WindowScope {
    ...
    private val delegate =
        object : NSObject(), NSWindowDelegateProtocol {
    ...
    private fun NSEvent.toComposeKeyEvent() =
        KeyEvent(
